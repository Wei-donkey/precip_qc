# -*- coding: utf-8 -*-
"""
Track and plot false negative cases from QC validation results.

Visualizes precipitation events where TRUE values were incorrectly classified as FALSE,
...
OUTPUT_DIR = SRC_DIR.parent / 'figures' / f"qc_false_tracking@nos_{DATASET}"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


def load_qc_rec(input_file: Path, df_qc_single: pd.DataFrame):
    """
    Load and filter records from INPUT_FILE_MORE that match the false negative cases 
    from the single circle experiment.
    """
    df = pd.read_csv(input_file, encoding='utf-8-sig')
    df['ddatetime'] = pd.to_datetime(df['ddatetime'])
    
    # Create merge keys from df_qc_fn_single
    df_fn_keys = df_qc_single[['stacode', 'ddatetime']].copy()
    df_fn_keys['stacode'] = df_fn_keys['stacode'].astype(str)

    df_matched = pd.merge(df, df_fn_keys, on=['stacode', 'ddatetime'], how='inner' )

    mask = (df_matched['validation'] == True) & (df_matched['qc_label'] == 'FALSE')
    df_matched.loc[mask,'confusion_type'] = 'False negative'

    mask = (df_matched['validation'] == True) & (df_matched['qc_label'] != 'FALSE')
    df_matched.loc[mask,'confusion_type'] = 'True positive'
...
def plot_qc_result(conn, total_records, all_stations, df_stations, flag: str,
    ...
    for idx, record in df_qc_result.iterrows():
        ...
        if confusion_type == 'False negative':
            label = 'fn'
        elif confusion_type == 'True positive':
            label = 'tp'
...
def main():
    ...
    df_qc_single = load_qc_result(INPUT_FILE_SINGLE,qc_type='false negative')
    df_qc_few = load_qc_rec(INPUT_FILE_FEW, df_qc_single)
    df_qc_many = load_qc_rec(INPUT_FILE_MORE, df_qc_single)
    ...
    with engine.connect() as conn:
        ...
        print(f"Plotting {total_records} false qc cases @ single circles...")
